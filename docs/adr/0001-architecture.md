...
> 정정(2026-08): 임베딩 어댑터는 이후 provider 선택형으로 확장되어, 로컬 e5(`intfloat/multilingual-e5-base`·768차원)를 기본으로 두고 OpenAI(1536)를 옵션으로 지원합니다. 아래 서술의 OpenAI·1536은 확정 당시 기준이며, 현행 기본값·차원은 e5·768입니다 (`src/agentory/core/config.py`·[docs/mcp/knowledge.md](../mcp/knowledge.md) 참고).

본 ADR §4에서 임베딩·벡터 스토어 포트를 3대 교체 지점으로 지정했고, 실제 어댑터는 확정
당시 pgvector·OpenAI 임베딩 단일 구현으로 두었습니다. 구현 중 다음을 추가로 결정했습니다.
...
- 임베딩 차원이 벡터 컬럼 스키마에 하드 커플링되어 있어, 임베딩 모델 교체 시 차원
