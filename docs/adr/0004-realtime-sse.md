...
전체 이력을 over-fetch하는 순차 왕복이라 캘린더 표시가 느렸습니다. 서버 기간 필터로 이 왕복을
없앱니다. 별도 "가용 날짜 집계" 엔드포인트도 검토했으나, 요구가 "선택 기간의 이력 조회"라 기존
목록 필터 확장으로 충분하고 `occurred_at::date` GROUP BY용 표현식 인덱스 추가를 피할 수 있어
채택하지 않았습니다.
