...
- **심각도 단일 소스 매뉴얼 정합**: 심각도 접두 규칙(ERR=위험) 대신 `telemetry/schemas.py`의
  ...
  `ANOMALY_SEVERITY`도 같은 함수를 써 WRN-901을 위험으로 판정합니다. (이 다변량 위험 분류는 §7에서
  주의로 재조정됩니다)
...
### 7. 다변량 이상 심각도 재분류: 위험 → 주의 (feature/254 #254)

§6에서 다변량 이상 코드(WRN-901)를 복합 냉각 고장(ERR-402)과 함께 위험으로 두었으나, 매뉴얼
정합을 다시 검토하면서 뒤집었습니다. 다변량 이상은 개별 센서가 모두 정상 밴드 안이면서 변수 간
관계만 어긋나는 상태라, 매뉴얼(EQP-ETCH-GDL-001 §3.1)과 실무 FDC 관행 모두 즉시 정지가 아니라
관찰 강화·엔지니어링 검토로 대응하는 주의(WRN 계열) 상태로 분류합니다. "즉시 정지가 아님"은
심각도를 낮추지 않고도 대응 수준에서 담기므로, 위험 심각도로 두면 오히려 즉시 정지 오탐을
유발한다고 판단했습니다.

`CRITICAL_ALARM_CODES`를 `{ERR-402}`만으로 축소해 복합 냉각 고장만 위험으로 남기고, WRN-901과
은퇴 코드 ERR-901은 주의로 판정합니다. `ANOMALY_SEVERITY`도 파생값이라 자동으로 주의가 됩니다.
아울러 RAG 지식베이스(GDL·SOP·MPS-8600)의 구 코드 ERR-901 서술을 실제 발령 코드 WRN-901로
정합해, WRN-901 발생 시 챗봇이 대응 카드를 검색하지 못하던 문제도 해소했습니다.

| 항목 | §6 시점 | §7 현행 |
| --- | --- | --- |
| 위험 코드셋 | ERR-402·ERR-901·WRN-901 | ERR-402 |
| 다변량 이상(WRN-901) 심각도 | 위험 | 주의 |
| 대응 수준 | 관찰 강화·엔지니어링 검토 | 관찰 강화·엔지니어링 검토 (동일) |
| RAG 매뉴얼 다변량 코드 | ERR-901 (위험) | WRN-901 (주의) |
