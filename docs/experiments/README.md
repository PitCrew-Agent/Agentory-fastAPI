...
| ID | 실험 | 산출물 |
| --- | --- | --- |
| EXP-000 | 고정 평가 세트 구축 (simulator 시나리오 기반 이상 주입) | data/eval/v1 |
| EXP-001 | 현행 규칙 레이어 단독 채점 (베이스라인) | 베이스라인 지표 |
| EXP-002 | PCA MSPC (T² + SPE) | v0 후보 |
| EXP-003 | ablation (윈도우 길이·오버랩, 피처, 임계 퍼센타일) | v0 확정 config |
| EXP-004 | TS2Vec + kNN 거리 | v1 후보 |
| EXP-005 | v0 vs v1 A/B + 설비별 캘리브레이션 | 채택 결정 |
