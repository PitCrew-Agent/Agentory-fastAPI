...
측정합니다. 아래는 2026-07-20 재측정 결과이며, 질의 4종 x 5회로 각 n=20입니다.
...
질의 유형별 분해, 품질 저하 여부 검증(실 MCP·DB·RAG 경유), Fast Router 축 분리 측정,
2026-07-15 최초 측정(질의 3종 x 3회) 대비 재측정 경위는 모두
[ADR-0009](../adr/0009-agent-hybrid-orchestration.md)에 있습니다. 집계값은 워크로드 구성에 따라
달라지므로(잡담 질의 포함 여부 등) 개선 폭을 인용할 때는 ADR-0009의 질의 유형별 표를 기준으로
삼으시기 바랍니다.
...
다시 보여줄래?" 등)는 진단 키워드가 없어 잡담으로 분류되어 데이터 수집이 생략됩니다. 적대적 케이스를
포함한 라벨 세트(n=18) 기준 diagnostic 재현율 0.667, 치명 오탐 4건입니다. 상세와 후속 과제는
[ADR-0009](../adr/0009-agent-hybrid-orchestration.md) 3-4절에 있습니다.
