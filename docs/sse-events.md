...
- `severity`: 알람 심각도 `주의`·`위험`, 서버가 코드셋(`CRITICAL_ALARM_CODES`=`ERR-402`·`ERR-901`·`WRN-901`)으로 판정한 값입니다. 복합·다변량 코드만 위험이고 단일 급성(`ERR-401`·`ERR-301`·`ERR-201`)은 주의라 접두만으로 재추론하면 어긋납니다. 프론트는 이 값을 그대로 사용합니다
