...
class Settings(BaseSettings):
    ...
    # bge=고품질, minilm=CPU 기본, none=미적용
    # 주의: CPU에서 bge(568M)는 과도한 지연과 메모리 사용 가능
    reranker_provider: str = "minilm"
    ...
    anomaly_detection_enabled: bool = True  # 스코어러 루프 활성 (모델 적재 후 켬)
    # 섀도우 모드, 실알람 미발령·관찰 저널만 기록 (BE_ANOM01_SHADOW01), 검증 후 false로 전역 실발령
    anomaly_shadow_mode: bool = True
    # 섀도우 모드라도 실발령할 설비 (데모 시연용), 이 목록만 실제 WRN-901 발령·나머지는 관찰 저널
    anomaly_realfire_equipment: frozenset[str] = frozenset({"EQP-A03"})
    ...
    # VAR 예측 잔차 보완 경로 (BE_ANOM01_VAR01), 상관 붕괴 조기 감지 (EXP-009)
    anomaly_var_enabled: bool = False  # 경로 활성, off면 기존 동작과 완전 동일
    anomaly_var_lag: int = 2  # 자기회귀 차수
