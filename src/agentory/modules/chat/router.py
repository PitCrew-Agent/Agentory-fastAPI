...
@router.post("/query", response_model=ApiResponse[ChatResponse], summary="Tory 질의 (비스트리밍)")
async def query(
    req: ChatRequest,
    ...
    answer = await service.run_query(
        SessionLocal,
        req.session_id,
        req.message,
        user_sub=user["email"],
...
@router.get(
    "/sessions",
    response_model=ApiResponse[list[ChatSessionSummary]],
...
async def list_sessions(
    ...
    return ApiResponse.ok(await service.list_sessions(session, user["email"]))
